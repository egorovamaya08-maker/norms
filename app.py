...
def check_toc(doc, start_idx):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        # Заголовки 1 уровня
        if is_heading_1 or original_smart_is_section_header(txt, doc) or re.match(r'^\d+\s+[А-ЯЁ]', txt_clean):
    ...
    # 4. Словарь заголовков для сравнения
    headers_in_text = {}
        ...
        if not is_level1 and not is_subsection:
            ...
            # Подпись таблицы
                ...
                if tbl_match:
                    ...
                    # +++ НОВЫЙ КОД: проверка пустых строк до и после названия таблицы +++
                    body_idx = para_to_body_idx.get(idx)
                    if body_idx is not None:
