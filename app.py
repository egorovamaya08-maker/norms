...
def group_issues(issues_list):
    ...
    cont_pattern = re.compile(r'^Таблица\s+(\d+(?:\.\d+)?)\s+–\s+проверьте\s+наличие\s+«Продолжение таблицы \1»\s+/\s+«Окончание таблицы \1»\s+при переносе на следующую страницу$')
...
def check_word_document(file):
    ...
    body_elems = list(doc.element.body)
    elem_to_body_idx = {elem: i for i, elem in enumerate(body_elems)}

    table_captions_info = []

    try:
        start_body_pos = elem_to_body_idx.get(doc.paragraphs[start_idx]._element, 0)
    except:
        start_body_pos = 0
    ...
    for idx in range(start_idx, end_idx):
        ...
        if not is_level1:
            if re.match(r'^\d+(?:\.\d+)+\s*[А-Яа-я]', norm_text):
                is_subsection = True
            else:
        ...
        if not is_level1 and not is_subsection:
            ...
            if is_figure:
                ...
                body_idx = elem_to_body_idx.get(p._element)
                ...
                tbl_match = re.match(r'Таблица\s+(?:№\s*)?(\d+(?:\.\d+)?)', norm_text)
                if tbl_match:
                    ...
                    if not re.match(r'Таблица\s+(?:№\s*)?\d+(?:\.\d+)?\s+[–—]\s+\S', norm_text):
        ...
        if is_level1:
            ...
            sub_name = re.sub(r'^\d+(?:\.\d+)+\s*', '', norm_text).strip()
            key = f"Подраздел «{sub_name[:50]}»"
            first_line = get_effective_first_line_indent(p)
            if abs(first_line) > 0.1:
                auto_issues.append(f"{key} – уберите абзацный отступ у заголовка (сейчас {first_line:.1f} см)")
            if not is_paragraph_bold(p):
