...
# ------------------------------------------------------------
# Главная проверка документа (исправленная логика для подразделов)
# ------------------------------------------------------------
def check_word_document(file):
    ...
    # Флаги для отслеживания последнего непустого абзаца (раздел/не раздел)
    ...
    for idx in range(start_idx, end_idx):
        ...
        # Обновляем флаг "последний непустой абзац был заголовком раздела" ПОСЛЕ обработки текущего,
        # но для проверки подраздела нам нужно знать состояние ДО текущего абзаца.
        # Поэтому сначала проверяем подраздел, используя prev_nonempty_was_section_header,
        # а потом обновляем этот флаг для следующего абзаца.

        # Проверка для подраздела
        if is_subsection:
            ...
            # Главное: проверка пустой строки перед подразделом
            # Ошибка только если перед подразделом была пустая строка И последний непустой абзац НЕ был заголовком раздела
        ...
        # Проверка для раздела (первый уровень)
        if is_level1:
            key = text[:80]
        ...
        # Для обычного текста (не заголовки) – проверяем списки, формулы, отступы
        if not is_level1 and not is_subsection:
            ...
            if is_list:
                if not marker_valid:
                    ...
                    if list_errors:
                        ...
                        list_errors = []
                prev_para_empty = False
                # обновим флаг: обычный текст не является заголовком раздела
            ...
            # Обычный текст (не заголовок, не подпись)
            key = norm_text[:50]
            first_line = get_effective_first_line_indent(p)
            if abs(first_line - 1.0) > 0.2:
        ...
        # После обработки текущего абзаца обновляем флаг prev_nonempty_was_section_header для следующей итерации
        # (только если текущий абзац не пустой)
        if text:
            if is_level1:
                prev_nonempty_was_section_header = True
            else:
                # Если текущий абзац не является заголовком раздела (подраздел, обычный текст, рисунок, таблица) – флаг = False
                prev_nonempty_was_section_header = False
            prev_para_empty = False
            prev_was_formula = False if not is_formula_or_equation(norm_text) else prev_was_formula
        else:
            prev_para_empty = True
