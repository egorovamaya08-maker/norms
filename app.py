...
def check_word_document(file):
    ...
    # ---------- 5. ОСНОВНАЯ ПРОВЕРКА (от start_idx) ----------
    figure_counter = 0
    ...
    # Множество для отслеживания уже встреченных подразделов (чтобы не проверять дважды)
    ...
    for idx, p in enumerate(doc.paragraphs):
        ...
        # ----- 5.2 ПОДРАЗДЕЛЫ (проверяем только если они не были в содержании) -----
        elif subsection.match(text):
            sub_text = text  # полный текст подраздела, например "1.1 Эволюция..."
            # Если такой подраздел уже встречался ранее (например, в содержании), то пропускаем проверку
            if sub_text in seen_subsections:
                # Это повторное появление – значит, основной текст, проверяем
                pass
            else:
                # Первое появление – скорее всего, в содержании, добавляем в множество и не проверяем
                seen_subsections.add(sub_text)
                # Также не проверяем отступы и пустые строки для содержания
                # Но нужно убедиться, что это действительно содержание: если idx < start_idx, то это содержание.
                # Однако мы уже пропустили всё до start_idx, значит, если мы дошли сюда, то это основной текст.
                # Но тогда почему мы не видели этот подраздел раньше? Потому что он может быть в содержании, которое мы пропустили.
                # Поэтому правильнее: проверяем ВСЕ подразделы, но перед проверкой смотрим, не находится ли этот абзац внутри блока содержания.
                # Проще: вообще не проверять подразделы, которые находятся до start_idx. Но мы уже пропустили всё до start_idx.
                # Значит, все подразделы, которые мы видим, находятся после start_idx и являются основными.
                # Однако в выводе видно, что подраздел "Эволюция..." присутствует, значит он находится после start_idx, но это явно содержание.
                # Следовательно, start_idx определён неправильно – он указывает на абзац, который находится раньше, чем нужно.
                # Значит, нужно пересмотреть определение start_idx.
                # Временно отключим эту логику и просто проверяем подразделы, но с дополнительным условием: если абзац имеет выравнивание не по левому краю или содержит номер страницы в конце, то пропустить.
                # Но это сложно.
                # Давайте вместо этого проверим, есть ли в конце абзаца номер страницы (цифра, возможно с точкой). Если есть – пропускаем.
                # Это характерно для содержания.
                pass
            
            # Временно: будем проверять все подразделы после start_idx, но с отсевом по наличию номера страницы в конце
            # Ищем в конце абзаца число, возможно с точкой, которое не является частью названия
            if re.search(r'\s\d+$', text) or re.search(r'\s\d+\.$', text):
                # Похоже на номер страницы – пропускаем проверку
                continue
            
            # Основная проверка подраздела
        ...
        # ----- 5.3 ОБЫЧНЫЙ ТЕКСТ -----
            ...
            # Проверяем, что это не часть содержания (нет номера страницы в конце)
            if re.search(r'\s\d+$', text) or re.search(r'\s\d+\.$', text):
                continue
            # Отступ 1,0
            if not pf.first_line_indent:
