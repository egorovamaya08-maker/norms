...
def is_real_table(table, doc):
    """Проверяет, является ли таблица реальной (не служебной)"""
    if len(table.rows) == 0:
        return False
    ...
    if len(table.rows) == 1 and len(table.columns) == 1 and len(total_text) < 20:
        return False
...
def check_word_document(file):
    ...
    # Собираем все таблицы верхнего уровня (не вложенные)
    all_main_tables = []
    for table in doc.tables:
        depth = get_table_depth(table)
        if depth > 0:  # Пропускаем вложенные таблицы
            continue
    ...
    # Находим все маркеры Продолжение/Окончание и определяем таблицы-продолжения
    ...
    # Определяем таблицы-продолжения (после маркеров)
