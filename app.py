...
def check_word_document(file):
    ...
    # Вычисляем доступную ширину для картинок (в EMUs)
    first_sec = doc.sections[0]
    page_width = first_sec.page_width if first_sec.page_width else Cm(21).emu
    l_margin = first_sec.left_margin if first_sec.left_margin else Cm(2).emu
    r_margin = first_sec.right_margin if first_sec.right_margin else Cm(2).emu
    max_usable_width = page_width - l_margin - r_margin

    # Собираем все элементы документа в линейный список, чтобы сохранить точный порядок
    body_elements = []
    for child in doc.element.body:
        if child.tag.endswith('p'):
            body_elements.append(('p', docx.paragraph.Paragraph(child, doc)))
        elif child.tag.endswith('tbl'):
            body_elements.append(('tbl', docx.table.Table(child, doc)))
    ...
    for idx, (typ, elem) in enumerate(body_elements):
        if typ == 'p':
            ...
            if is_main_heading:
                # Проверка на перенос на новую страницу (игнорируем самый первый абзац документа)
                ...
                if has_text_before:
                    if p_format.page_break_before:
                        ...
                        # Проверяем ручные разрывы в текущем или предыдущем абзаце
                ...
                # У заголовков разделов не должно быть абзацного отступа
            ...
            # Проверка подразделов (1.1, 1.2, 2.1 и т.д.)
                ...
                # Проверка на пустую строку ПЕРЕД подразделом
            ...
            # Проверка подписей к Рисункам
                ...
                if elem.alignment != WD_ALIGN_PARAGRAPH.CENTER:
                    issues.append(f"{fig_label} - выровняйте подпись по тему центру")
                
                # Проверка заглавной буквы в названии
                ...
                # Проверка пустых строк вокруг рисунка/картинки
                has_prev_empty = False
                if idx > 0 and body_elements[idx-1][0] == 'p' and not body_elements[idx-1][1].text.strip():
                    has_prev_empty = True
                # Если перед подписью идет сам графический объект, смотрим перед ним
            ...
            # Проверка выхода картинки за границы полей
            if drawings:
                for ext in drawings:
            ...
            # Логика детекции заголовков Таблиц
            ...
            # Правила для элементов Списка литературы / источников
        ...
        # Проверка структуры самой таблицы
        elif typ == 'tbl':
            # Анализируем свойства таблицы, только если прямо перед ней встретили валидный заголовок
            if current_table_id:
                bold_inside = False
                ...
                current_table_id = None # Сбрасываем маркер
